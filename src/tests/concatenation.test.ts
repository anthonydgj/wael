...
test('should concatenate geometries', () => {
    ...
    expect(result).toBeTruthy();
    expect(result.geometry.geometries.map((f: any) => f.geometry.coordinates)).toStrictEqual([[1, 1], [2, 2], [3, 3], [4, 4]]);
    ...
    expect(result).toBeTruthy();
    expect(result.geometry.geometries.map((f: any) => f.geometry.coordinates)).toStrictEqual([[1, 1], [2, 2]]);
